...
sp=read.csv("techpowerup_enterprise_ssds.csv", as.is=TRUE)
...
plot(sp$date, sp$max_capacity_GB, log="y", col=pal_col[2],
     yaxt="n",
     xaxs="i",
     xlim=c(start_date, end_date), ylim=c(1000, 1e5),
     xlab="Release date", ylab="Capacity (GB)")

mem_vals=c("100", "1000", "10000","100000")
axis(2, at=as.numeric(mem_vals), label=mem_vals)

# lines(loess.smooth(sp$date, sp$max_capacity_GB, span=0.3), col="yellow")
# 
# loess_mod=loess(log(sp$max_capacity_GB) ~ sp$date, span=0.3)
# loess_pred=predict(loess_mod)
# lines(sp$date, exp(loess_pred), col=loess_col)

# sp_mod=glm(log(max_capacity_GB) ~ date, data=sp)
# summary(sp_mod)
# 
...
#rq98_mod=rq(log(max_capacity_GB) ~ date, data=sp, tau=0.98) # tau is the quantile
#pred=predict(rq98_mod, newdata=data.frame(date=u_date))
#lines(u_date, exp(pred), col=pal_col[1])

rq95_mod=rq(log(max_capacity_GB) ~ date, data=sp, tau=0.95) # tau is the quantile
pred_95=predict(rq95_mod, newdata=data.frame(date=u_date))
lines(u_date, exp(pred_95), col=pal_col[1])

rq50_mod=rq(log(max_capacity_GB) ~ date, data=sp, tau=0.50) # tau is the quantile
# summary(rq50_mod)
# 
